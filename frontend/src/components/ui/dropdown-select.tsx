    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      <PopoverTrigger asChild>
        <button
          ...
          disabled={disabled}
          className={cn(
            "motion-panel inline-flex h-10 w-auto cursor-pointer items-center justify-between gap-2 rounded-xl border border-[color:var(--border)] bg-white/80 px-3 py-2 text-sm font-medium text-strong shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-[color:var(--accent)] hover:bg-white focus:outline-none focus:ring-2 focus:ring-[color:var(--accent)] focus:border-transparent disabled:cursor-not-allowed disabled:opacity-50",
            open && "border-[color:var(--accent)] bg-white",
            triggerClassName,
          )}
        >
          <span
          ...
          <ChevronDown
            className={cn(
              "h-4 w-4 shrink-0 text-quiet transition-transform duration-200",
              open && "rotate-180",
            )}
          />
        </button>
      ...
      <PopoverContent
        ...
        sideOffset={6}
        className={cn(
          "w-[var(--radix-popover-trigger-width)] min-w-[12rem] overflow-hidden rounded-2xl border border-[color:var(--border)] p-0 text-strong shadow-lush",
          contentClassName,
        )}
      >
            ...
            {options.map((option) => {
              ...
              return (
                <CommandItem
                  ...
                  className={cn(
                    "flex items-center justify-between gap-2 rounded-xl px-4 py-3 text-sm text-[color:var(--text-muted)] transition-all duration-150 data-[selected=true]:bg-[linear-gradient(135deg,rgba(8,145,178,0.12)_0%,rgba(34,211,238,0.1)_100%)] data-[selected=true]:text-[color:var(--accent-strong)]",
                    isSelected && "font-semibold",
                    !isSelected && "hover:bg-white/75 hover:text-[color:var(--accent)]",
                    itemClassName,
                  )}
                >
                  <span className="flex min-w-0 items-center gap-2">
                    {OptionIcon ? (
                      <OptionIcon
                        className={cn(
                          "h-4 w-4",
                          isSelected ? "text-[color:var(--accent-strong)]" : "text-quiet",
